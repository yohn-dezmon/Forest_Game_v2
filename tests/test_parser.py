...
# if I input unknown words into lexicon, what kind of output will I get??
word_list_not_in_dic = [('verb','launch'),('noun','tomato')]
...
def test_sentence(fixtcha_):
    ...
    result_VO = parser_VO.parse_sentence()
    result_SVO = parser_SVO.parse_sentence()

    assert result_VO.subject == 'player'
    assert result_VO.verb == 'kill'
    assert result_VO.object == 'cabinet'
    # should pass!
    assert result_SVO.subject == 'player'
    assert result_SVO.object == 'cabinet'
    assert result_SVO.verb == 'kill'
